...
col_association = function(x){
  cols=c()
  for (i in x){
    if(i=="sensitivity"||i=="sensitivity "||i=="response"||i=="response "||i=="increased benefit"||i=="sensitivity/response"){
      cols=c(cols,paste("\\textcolor[HTML]{388E3C}{", formatC(i, dig=2, format="f"), "}"))
    }
    if(i=="resistance"||i=="resistance "||i=="no response"||i==" no response"||i=="resistance or non-response"||i=="no sensitivity"||i=="decreased sensitivity"||i=="reduced sensitivity"||i=="adverse response"){
      cols=c(cols,paste("\\textcolor[HTML]{C62828}{", formatC(i, dig=2, format="f"), "}"))
    }
    if(is.na(i)||i=="n/a"||i=="N/A"||i==""){
      cols=c(cols,paste("\\textcolor[HTML]{388E3C}{", formatC(i, dig=2, format="f"), "}"))
    }
  }
 return(cols)
...
tx     <- UPLOAD3()
A      <- DruggableUPLOAD()[[4]]
druggableTARGET <- DruggableUPLOAD()[[1]]
v_gdkd <- UPLOAD_DB()[[3]]
v_civic <- UPLOAD_DB()[[4]]
...
%\hline
\textbf{Patient ID}         & \Sexpr{ID}                   & \textbf{Tissue Type}        &  \Sexpr{input$tissue}    \\
\textbf{Gender}             & \Sexpr{input$gender}         & \textbf{Tumor Content (\%)} & \Sexpr{input$tumor_content}    \\ 
\textbf{Disease}            & \Sexpr{cancer}               & \textbf{Number SNVs}        &  \Sexpr{nrow(maf)}          \\ 
\textbf{Stage}              & \Sexpr{input$stage}          & \textbf{Number CNVs}        &  \Sexpr{nrow(cnv)}  \\
\textbf{Previous Therapies} & \Sexpr{input$prev_ther}      & \textbf{Number Fusions}     &  \Sexpr{nrow(tx)}   \\
\end{tabular}
\end{table}
...
%\vspace{0.3cm}
\textbf{Method:} Somatic variants of the patient (mutations, amplifications, deletions, rearrangments) are searched in curated databases of predictive biomarkers (GKDB\footnote{Dienstmann et al., Cancer Discov (2015), \Sexpr{v_gdkd}}, CIViC\footnote{Griffith et al., Nat Genet (2017), \Sexpr{v_civic}}) and reported according to their clinical evidence (see Levels of Evidence). In the following two tables (SNVs and CNVs), basic information of the somatic variants with relevant clinical implications can be found: \\
...
for (drug in names(sort(table(d),decreasing = TRUE))){
  index <- grep(drug,A2[,6],ignore.case = T)
  index <- index[!(index %in% final_order)]
  final_order <- c(final_order,index)
}
...
align(x)<-"p{0.1in}p{0.4in}p{0.4in}|p{0.65in}p{0.7in}p{0.7in}p{1.3in}p{0.6in}p{0.7in}p{0.2in}"
